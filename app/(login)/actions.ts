...
// Renomeie a função duplicada
export async function removeTeamMemberAction(prevState: any, formData: FormData) {
  const memberId = formData.get('memberId');
  if (!memberId) {
    return { error: 'Member ID is required' };
  }

  // Implemente a lógica para remover o membro da equipe aqui
  // Por exemplo:
  // await db.delete(teamMembers).where(eq(teamMembers.id, Number(memberId)));
